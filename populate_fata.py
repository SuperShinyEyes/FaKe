...
def add_product(categories, seller, name, product_num, price, stock, product_info, status):
  g = Product.objects.get_or_create(seller=seller, name=name, product_num=product_num, price=price, stock=stock, product_info=product_info, status=status)[0]
  for c in categories:
    g.categories.add(c)
  g.save()
  return g
...
def populate():
  ...
  add_product((cs[4],cs[5]), sellers[0], name='American film', product_num='2', price=300, stock=10, product_info="Everything about American film", status=True)
  add_product((cs[1],cs[2]), sellers[2], name='European women', product_num='4', price=120, stock=10, product_info="Everything about European women", status=True)

#print Product.objects.all()[0].categories.all()[0].category_name
